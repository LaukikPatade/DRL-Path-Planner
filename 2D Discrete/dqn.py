...
class CropFieldDQL():
    ...
    def train(self, episodes, render=False, is_slippery=False, map="4x4"):
        writer = SummaryWriter(f'./runs_trailing/dqn_{map}')
        ...
        # Save plots
        if is_slippery:
            plt.savefig(f'noisy_dqn_{map}.png')
            torch.save(policy_dqn.state_dict(), f"noisy_dqn_{map}.pt")

        else:
            plt.savefig(f'dqn_{map}.png')
            torch.save(policy_dqn.state_dict(), f"dqn_{map}.pt")
    ...
    def test(self, episodes, is_slippery=False, map="4x4"):
        ...
        if is_slippery:
            policy_dqn.load_state_dict(torch.load(f"noisy_dqn_{map}.pt"))
        else:
            policy_dqn.load_state_dict(torch.load(f"../models/dqn/dqn_{map}.pt"))
