...
class DuelingDQN(nn.Module):
    def __init__(self, in_states, h1_nodes, out_actions):
        super().__init__()
        self.input_features = in_states
        self.hidden_state = h1_nodes
        self.output_features = out_actions
        ...
        self.advantage = nn.Linear(h1_nodes, out_actions)
        self.value = nn.Linear(h1_nodes, 1)
  
    def forward(self, x):
        hidden_out = self.hidden(x.float())
        advantage = self.advantage(hidden_out)
        value = self.value(hidden_out)
...
class CropFieldDQL():
    ...
    def train(self, episodes, render=False, is_slippery=False, map="4x4"):
        writer = SummaryWriter(f'./runs_trailing/d3qn_{map}')
        ...
        policy_dqn = DuelingDQN(in_states=num_states, h1_nodes=num_states, out_actions=num_actions)
        target_dqn = DuelingDQN(in_states=num_states, h1_nodes=num_states, out_actions=num_actions)
        ...
        # Save plots
        if is_slippery:
            plt.savefig(f'noisy_d3qn_{map}.png')
            torch.save(policy_dqn.state_dict(), f"noisy_d3qn_{map}.pt")

        else:
            plt.savefig(f'd3qn_{map}.png')
            torch.save(policy_dqn.state_dict(), f"d3qn_{map}.pt")
    ...
    def optimize(self, mini_batch, policy_dqn, target_dqn):
        ...
        num_states = policy_dqn.input_features
    ...
    def test(self, episodes, is_slippery=False, map="4x4"):
        ...
        if is_slippery:
            policy_dqn.load_state_dict(torch.load(f"./models/d3qn/noisy_d3qn_{map}.pt"))
        else:
            policy_dqn.load_state_dict(torch.load(f"./models/d3qn/d3qn_{map}.pt"))
