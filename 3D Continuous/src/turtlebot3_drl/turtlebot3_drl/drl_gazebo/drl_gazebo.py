...
class DRLGazebo(Node):
    ...
    *******************************************************************************"""
    ...
    def goal_is_valid(self, goal_x, goal_y):
        # Check if the goal is within the arena boundaries
        if goal_x > ARENA_LENGTH / 2 or goal_x < -ARENA_LENGTH / 2 or goal_y > ARENA_WIDTH / 2 or goal_y < -ARENA_WIDTH / 2:
            return False

        # Check if the goal is too close to any circular obstacle
        for obstacle in self.obstacle_coordinates:
            center_x, center_y = obstacle["center"]
            radius = obstacle["radius"]

            # Calculate the distance between goal and obstacle center
            distance = ((goal_x - center_x) ** 2 + (goal_y - center_y) ** 2) ** 0.5

            # If the distance is less than the radius, the goal is invalid
            if distance < radius + NO_GOAL_SPAWN_MARGIN:  # Add margin if needed
                return False

        # If the goal passes all checks, it is valid
        return True
    ...
    def get_obstacle_coordinates(self):
        tree = ET.parse(os.getenv('DRLNAV_BASE_PATH') + '/src/turtlebot3_simulations/turtlebot3_gazebo/models/turtlebot3_drl_world/inner_obstacles_cylinder/model.sdf')
        root = tree.getroot()
        print('\n\n  root ---->',root.find('model').findall('link'))
        obstacle_coordinates = []
        for model in root.findall('model'):
            for link in model.findall('link'):
                # Extract the pose of the cylinder
                pose = link.find('pose').text.split(" ")
                pose_x = float(pose[0])
                pose_y = float(pose[1])
                pose_z = float(pose[2])  # You can use pose_z if needed

                # Extract cylinder geometry (radius and length)
                cylinder = link.find('collision').find('geometry').find('cylinder')
                radius = float(cylinder.find('radius').text)
                length = float(cylinder.find('length').text)  # Not used for 2D, but available

                # Store the cylinder's center and radius
                obstacle_coordinates.append({
